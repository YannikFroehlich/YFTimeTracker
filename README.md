...
- Optionaler Kontoabgleich mit der YFDatenbank: Anmeldung über das Profil oben rechts, danach werden Spiele, EXE-Zuordnungen, Sessions, Tags, Ausschlüsse, Cover und Einstellungen in einem Supabase-Konto gespeichert. Auf einem zweiten PC zeigt dieselbe Anmeldung dieselbe Bibliothek und alle Spielzeiten zusammengeführt. Der Abgleich läuft beim Start sowie bei jedem Spielstart und -ende automatisch und lässt sich jederzeit von Hand anstoßen; eine Session wandert erst ins Konto, wenn sie beendet ist; Zugangsdaten liegen im Windows-Anmeldeinformationsspeicher und nie in Exporten. Details: [docs/SUPABASE_SETUP.md](docs/SUPABASE_SETUP.md)
- Website [tracker.yfserver.de](https://tracker.yfserver.de) mit demselben Konto: Spieler und Spiele suchen, Bestenlisten, Spieleseiten und das eigene Profil. Jedes Konto ist privat, bis man dort einen Benutzernamen wählt und das Profil öffentlich schaltet; öffentlich sind nur Summen, keine einzelnen Sessions, Geräte oder Pfade
